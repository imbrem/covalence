...
([`observers.md`](../observers/observers.md)): instead of a validator _trusting_ a backend, we
...
being formalized" claim of [`VISION.md`](../vision/VISION.md).
...
- This realizes the **reverse-math** discipline of the VISION doc: prove
  ...
  [`surface-syntax.md`](../surface/surface-syntax.md) §6.
...
built over the [monoid theory](../surface/surface-compiler.md) (`init/monoid.rs`):
...
- Building a derivation and checking `Derivable_X ⌜A⌝` are the _same
  ...
  rules.
- These constructors are written in the [surface syntax](../surface/surface-syntax.md)
...
[`surface-syntax.md`](../surface/surface-syntax.md) §1.1: we want the surface
...
gives everything a common semantics ([`kernel-design.md`](../kernel/kernel-design.md)
...
([`surface-compiler.md`](../surface/surface-compiler.md), §8) — then build a rich
...
(the [`frontend.md`](../surface/frontend.md) handler dispatch) later powers
**accelerated term-lists and term-sets** (efficient `covalence-pure`
observers over collections of terms, [`observers.md`](../observers/observers.md) §7)
...
[`surface-syntax.md`](../surface/surface-syntax.md) §1.1). A wrong or slow handler
...
The `.cov` script layer is the first, deliberately-small
...
> scaled to the **PA deep embedding** (`crates/kernel/hol/init/src/peano/`,
...
(`surface/mod.rs`, [`surface-syntax.md`](../surface/surface-syntax.md)) is where a
term will be carried _together with its interpretations_, letting the user
ask the entailment questions of [`surface-syntax.md`](../surface/surface-syntax.md)
...
- **Everything-reified-in-HOL.** The kernel is HOL with conservative
  ...
  ([`kernel-design.md`](../kernel/kernel-design.md)). The single biggest enabler —
