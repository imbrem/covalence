...
fn nat_rec_predicate() -> Term {
    ...
    let step = logic::hol_forall("n", Type::nat(), step_eq);

    let body = logic::hol_and(base_eq, step);
    let body_zf = logic::hol_forall("z", alpha, logic::hol_forall("f", f_ty, body));
...
fn nat_add_body() -> Term {
    ...
    iter_binary(Term::succ(), Term::free("m", Type::nat()))
}
...
fn nat_cmp_predicate(zero_zero: bool, zero_succ: bool, succ_zero: bool) -> Term {
    ...
    let succ_m = Term::app(Term::succ(), m.clone());
    let cmp_0_succm = hol::hol_eq(
        Term::app(Term::app(cmp.clone(), zero.clone()), succ_m.clone()),
        lit(zero_succ),
    );
    let cmp_0_succ = logic::hol_forall("m", Type::nat(), cmp_0_succm);
    ...
    let cmp_succ_0 = logic::hol_forall("n", Type::nat(), cmp_succn_0);

    let n2 = Term::free("n", Type::nat());
    let m2 = Term::free("m", Type::nat());
    let succ_n2 = Term::app(Term::succ(), n2.clone());
    let succ_m2 = Term::app(Term::succ(), m2.clone());
    let cmp_succ_succ = hol::hol_eq(
        Term::app(Term::app(cmp.clone(), succ_n2), succ_m2),
        Term::app(Term::app(cmp.clone(), n2), m2),
    );
    let cmp_ss_m = logic::hol_forall("m", Type::nat(), cmp_succ_succ);
    let cmp_ss = logic::hol_forall("n", Type::nat(), cmp_ss_m);

    let body = logic::hol_and(
        cmp_00,
        logic::hol_and(cmp_0_succ, logic::hol_and(cmp_succ_0, cmp_ss)),
    );
