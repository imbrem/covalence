/*
 * Vendored from BLAKE3 official C reference at tag 1.8.5
 *   https://github.com/BLAKE3-team/BLAKE3/blob/1.8.5/c/blake3.h
 * Upstream commit: 93a431c78a52d7ccf0f366f106467f5070e6075e
 * License: CC0 1.0 OR Apache-2.0 (public domain at the CC0 layer).
 *
 * Pristine apart from header substitutions for the freestanding
 * wasm32-unknown-unknown build: `<stddef.h>` / `<stdint.h>` come from
 * clang's freestanding builtins, so those includes stay. Other files
 * that pulled in `<string.h>` / `<assert.h>` route through cov_compat.h
 * (declared in main.c) instead. Only the portable backend is reachable
 * on wasm32 (no IS_X86 / IS_AARCH64 path).
 */
