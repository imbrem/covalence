...
- Four non-computational primitive **rules**: **Peano induction on
  `nat`** (`Thm::nat_induct`, connective-free **sequent form**:
  `Γ_b ⊢ p[0/x]` + `Γ_s ⊢ p[succ x/x]` with `p ∈ Γ_s` ⟹
  `Γ_b ∪ (Γ_s \ {p}) ⊢ p`, `x : nat` free; the formula form
  `base ⊢ p 0` + `step ⊢ p n ⟹ p (succ n)` ⟹ `⊢ ∀n. p n` is a
  derivation, `covalence-init`'s `init::ext::nat_induct`), **ex falso**
...
```rust
Thm::nat_induct(base, step, p, x) -> Result<Thm>
    // Sequent form (connective-free): p : bool with x : nat free.
    // base : Γ_b ⊢ p[0/x]   step : Γ_s ⊢ p[succ x/x] with p ∈ Γ_s
    //   ⟹  Γ_b ∪ (Γ_s \ {p}) ⊢ p    (x free — universal by genericity)
    // The kernel COMPUTES p[0/x] / p[succ x/x] (subst_free; succ =
    // hol::succ_fn) and matches them syntactically. Side condition
    // (soundness-critical): x not free in Γ_s \ {p}; x MAY be free in
    // Γ_b (the base is only consumed at the ambient valuation, never
    // re-instantiated). Sound because Type::nat() denotes the standard
    // naturals generated by 0 and succ — full valuation argument in the
    // method docstring. The old formula form (base ⊢ p 0, step ⊢
    // p n ⟹ p (succ n) ⟹ ⊢ ∀n. p n) is a derivation over this rule +
    // assume/imp_elim/inst/all_intro: `covalence-init::init::ext::nat_induct`,
    // a drop-in same-signature replacement.
