...
impl Prims {
    ...
    /// `⊢ aplus x y = aint (intAdd (intval x) (intval y))` — the `+`
    /// unfolding at fixed terms (internal seam for the S5c typed-
    /// arithmetic pack, the `alt_unfold` pattern).
    pub(crate) fn plus_unfold(&self, x: &Term, y: &Term) -> Result<Thm> {
        apply_def(&self.plus_eq, &[x.clone(), y.clone()])
    }

    /// `⊢ aneg x = aint (intNeg (intval x))` — the `unary--` unfolding at
    /// a fixed term.
    pub(crate) fn neg_unfold(&self, x: &Term) -> Result<Thm> {
        apply_def(&self.neg_eq, std::slice::from_ref(x))
    }
