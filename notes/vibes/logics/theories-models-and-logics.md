...
> [`pure-design.md`](../kernel/pure-design.md) (the assumption / meta-assumption
> sets).
...
(`pure-design.md §3`) surfaced as a `Model` responsibility. This
...
`discharge(NatAccel)` from `pure-design.md §4.2`. So three things we designed
...
`compile_project` ([`cov-project.md`](../web/cov-project.md)) builds, replacing the hand-written
...
**scoped to a theory** ([`VISION.md`](../vision/VISION.md), metatheory-as-default). The outer HOL is
...
- `compile_project` ([`cov-project.md`](../web/cov-project.md)) — the multi-file `.cov` DAG builder
...
1. **Fix the waist — and at HOL Light (rank-1) or HOL-ω?** **RESOLVED:** the
   *final* waist is **HOL-ω** (to program the middle language like Haskell — the
   ...
   [`kernel-design.md §11.6`](../kernel/kernel-design.md).
