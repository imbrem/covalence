...
2. **Term hash-consing not threaded through the inference rules** — proofs don't
   ...
   `list.cov`/`utf8.cov`/regex blow-ups. [`covalence-core`](crates/covalence-core/SKELETONS.md), [`script` perf](crates/covalence-hol/src/script/SKELETONS.md)
3. **`.cov` script async core + source spans missing** — holes/channels deleted,
   ...
   LLM-assisted proofs and the project loader's concurrent compile. [`script`](crates/covalence-hol/src/script/SKELETONS.md)
4. **PA derivation constructors (quantifier/induction/Leibniz)** — the
   ...
   constructed *through* the clauses yet. [`peano`](crates/covalence-hol/src/peano/SKELETONS.md)
5. **Metalogic structural `σ` transport + set.mm-scale rule sets** — transport
   ...
   (HOL→ZFC, `Metamath-L ≅ native-L`) waits on a reified inductive encoding. [`metalogic`](crates/covalence-hol/src/metalogic/SKELETONS.md)
...
7. **`list` recursion cons-side + nat Euclidean division** — gate `list_foldl`,
   `map`/`filter`, all `bytes`/`string`/text length/index, and `div_mod`/`mod_lt`. [`init`](crates/covalence-hol/src/init/SKELETONS.md)
8. **CFG stratum for grammars** — SpecTec/regex front ends cover only the regular
   base case; `Var` non-terminals rejected, blocking full WASM binary grammar. [`hol/spectec`](crates/covalence-hol/src/spectec/SKELETONS.md), [`covalence-spectec`](crates/covalence-spectec/SKELETONS.md)
9. **`rat`/`real` ordered-field postulates pending proof** — `mul_inv`, `le_def`,
   Dedekind-cut suprema still `axiom`-postulated. [`init`](crates/covalence-hol/src/init/SKELETONS.md)
