;; Store overwrite: set same key twice, verify second value wins.
(component
    ...
    (core module $m
        ...
        ;; Data: key = "k" at 0, val1 = [10] at 4, val2 = [20] at 8
        (data (i32.const 0) "k")
        (data (i32.const 4) "\0a")
        (data (i32.const 8) "\14")
        ...
        (func $start
            ...
            ;; set("k", [20]) — overwrite
            (call $store_set (local.get $handle) (i32.const 0) (i32.const 1) (i32.const 8) (i32.const 1))
            ;; get("k") → retptr at 64
            ...
            ;; Check value == 20
            (if (i32.ne (i32.load8_u (i32.load (i32.const 64))) (i32.const 20))
                (then (unreachable))
            )
