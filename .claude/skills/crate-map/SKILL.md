...
- **covalence-core** — **the pure-HOL tier of the TCB, together with `covalence-pure(-trusted)`** (safe Rust; `core::Thm<L: HolTier = CoreLang>` is a newtype over `pure::Thm` — see `docs/deps/tcb.json`; `Thm<CoreLang>` carries NO computation axioms). HOL-Light-style kernel, locally-nameless `Term`/`Type`. Logical primitives are only `=` (`TermKind::Eq`) and `ε` (`TermKind::Select`); connectives are defined constants in `defs/logic.rs`. HOL Light's equality-core primitives + a few fast derived constructors with `Soundness:` docstrings (26 rules, golden `docs/deps/core-manifest.txt` — the connective/quantifier rules and `lem` left in stage L2: they are zero-TCB derivations in `covalence-hol-eval::derived::DerivedRules`, same signatures); `define` + `new_type_definition` (returns the bijection as ONE unsplit conjunction — split via eval's `TypeDefExt`); `unfold_term_spec`; `spec_abs`/`spec_rep`. Three non-computational primitive rules: `nat_induct` (sequent form), `false_elim`, `unit_eq`. `src/defs/` is only the **D3 residue** (literal-leaf type handles + connective builders — dies with the literal leaves; see its `SKELETONS.md`): the term-op catalogue and the cert rules moved to `covalence-hol-eval` (stage E2). **Read `notes/vibes/kernel-design.md` before touching.**
