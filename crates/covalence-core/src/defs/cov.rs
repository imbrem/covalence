...
// ============================================================================
// Env — the non-lazy core catalogue
// ============================================================================
...
/// The process-shared **core catalogue** [`Env`], built by parsing the
/// embedded [`CORE_COV`] source once. Each entry's kernel objects are
/// the same `Arc`-shared accessors as the hand-written `defs::*` (the
/// catalogue resolver returns the cached accessors), so this `Env` is a
...
pub fn core_env() -> &'static Env {
    use std::sync::LazyLock;
    static CORE: LazyLock<Env> =
        LazyLock::new(|| parse_core(CORE_COV).expect("embedded core.cov must parse"));
    &CORE
}

/// Parse a whole `.cov` source string into an [`Env`].
pub fn parse_core(src: &str) -> R<Env> {
...
pub fn term_str(env: &Env, src: &str) -> R<Term> {
    let forms = covalence_sexp::parse(src).map_err(|e| CovError::Sexp(e.to_string()))?;
    let [form] = &forms[..] else {
...
pub fn type_str(env: &Env, src: &str) -> R<Type> {
    let forms = covalence_sexp::parse(src).map_err(|e| CovError::Sexp(e.to_string()))?;
    let [form] = &forms[..] else {
...
fn directive(form: &SExpr, env: &mut Env) -> R<()> {
...
/// stored directly.
fn insert_type(env: &mut Env, name: String, fresh: TypeSpec) -> R<()> {
...
fn parse_type(s: &SExpr, env: &Env) -> R<Type> {
    match s {
...
fn parse_type_atom(name: &str, env: &Env) -> R<Type> {
    match name {
        "bool" => Ok(Type::bool()),
        "nat" => Ok(Type::nat()),
        "int" => Ok(Type::int()),
...
fn apply_type_ctor(name: &str, args: Vec<Type>, env: &Env) -> R<Type> {
...
fn term(s: &SExpr, scope: &mut Scope, env: &Env) -> R<Term> {
    match s {
...
fn term_atom(name: &str, scope: &mut Scope, env: &Env) -> R<Term> {
    // (0) bool literals.
    match name {
    ...
    //
    // Catalogue resolution comes *before* the `Env` lookup so that a
...
fn parse_lam(ch: &[SExpr], scope: &mut Scope, env: &Env) -> R<Term> {
    arity(ch, 3, "#lam")?;
...
fn parse_sel(ch: &[SExpr], scope: &mut Scope, env: &Env) -> R<Term> {
    arity(ch, 3, "#sel")?;
    let (name, ty) = parse_binder(&ch[1], env)?;
    scope.push(name.clone(), ty.clone());
...
/// function.
fn parse_coercion(ch: &[SExpr], scope: &mut Scope, env: &Env, is_abs: bool) -> R<Term> {
...
fn parse_type_app(ch: &[SExpr], env: &Env) -> R<Term> {
    if ch.len() < 2 {
...
/// `(x A)` — a typed binder.
fn parse_binder(s: &SExpr, env: &Env) -> R<(String, Type)> {
...
fn type_arg_list(s: &SExpr, env: &Env) -> R<Vec<Type>> {
    let ch = list(s, "type-arg list")?;
    ch.iter().map(|c| parse_type(c, env)).collect()
}

fn resolve_type_spec(name: &str, env: &Env) -> R<TypeSpec> {
    if let Some(spec) = env.type_spec(name) {
...
mod tests {
    ...
    fn migrated_terms_are_byte_identical() {
        ...
        // Each `#def` body stored in the `Env` is byte-identical (`==`)
