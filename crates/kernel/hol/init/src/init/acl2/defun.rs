...
/// Names a defun may not shadow / use as formals. `b`/`h`/`t` are the
/// carrier `induct` binder hints (they name the case variables of the
/// `def_eq_model` proof), so formals may not collide with them.
fn check_names(env: &Acl2Env, spec: &DefunSpec) -> Result<()> {
    if spec.name.is_empty() || spec.name == "QUOTE" || spec.name == "IF" {
        return Err(bad(format!("illegal defun name `{}`", spec.name)));
    }
    if env.rows.iter().any(|r| r.sym == spec.name) {
        return Err(bad(format!(
            "name `{}` collides with an existing table row",
            spec.name
        )));
    }
    for (i, f) in spec.formals.iter().enumerate() {
        if f.is_empty() {
            return Err(bad("empty formal name"));
        }
        if spec.formals[..i].contains(f) {
