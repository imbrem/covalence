...
- **covalence-grammar** — grammar/parsing support (used by hol + spectec). Includes the compositional numeral-literal grammars (`src/numeral.rs`: `Numeral<BASE>`/`nat_any`/`int_any`/`frac`/`decimal`/`sci` + deparsers) producing `covalence-types` `Nat`/`Int`/`Rat`/`Decimal`.
- **covalence-numerals** (`crates/lang/numerals`) — the numeral-literal ladder behind a **swappable backend**. Two traits: `LiteralGrammar` (parse/deparse; default `NumeralGrammar` wraps covalence-grammar) and `NumeralBackend` (`type Repr`/`Thm`; `nat`/`int`/`decimal`/`rat`/`f32_bits` builders + `prove_eq`/`prove_lt`/`prove_to_rat`/`ground_f32`); `lower<G,B>` glues them. Default build is kernel-free (`RatBackend`). Behind the `hol` feature: `Symbolic` (structural — proves `0xABC = 2748` by folding both to a common kernel-literal normal form and chaining `refl`/`reduce`/`trans`, **no new TCB**), `Builtin` (same via kernel `reduce`/`prove_true` CanonRule path, agrees with `Symbolic`), `Wasm` (SKELETON). `prove_to_rat` discharges the proved `init::decimal::to_rat_at` injection lemma. `ground_f32` enclosure cert unproven (SKELETONS).
