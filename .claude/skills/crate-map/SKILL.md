...
- **covalence-k** (`crates/lang/k`) — the K-framework frontend, slice 1: consumes **KORE** (the elaborated matching-logic IR kompile emits as `definition.kore`) exactly as covalence-spectec consumes the elaborated SpecTec AST instead of `.watsup`. Untrusted and kernel-free (deps: covalence-sexp + smol_str + thiserror only): hand-rolled recursive-descent KORE parser (`parse`; handles K's mangled identifiers, multiary `\and`/`\or`, sortless `\mu`/`\nu`), plain-data AST (`ast`), ONE canonical deterministic S-expression rendering (`sexpr`, to_sexp-only), and the fragment classifier (`fragment`: rewrite/function/subsort/constructor/simplification/ceil/assoc-comm ladder, `RewriteRule` extraction with requires/ensures/priority/label/unique-id). Lowering into `covalence-init` is a later slice; design `notes/design/k-frontend.md`.
