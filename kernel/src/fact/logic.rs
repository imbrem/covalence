...
{
    fn check_in(self, ctx: C, db: &R) -> bool {
        self.is_none_or(|f| f.check_in(ctx, db))
    }
}
...
where
    ...
    Q: Copy,
{
    fn check_under(self, _ctx: C, _binder: Q, _db: &R) -> bool {
        true
    }
}

impl<C, Q, R> FactUnder<C, Q, R> for bool
where
    C: Copy,
    Q: Copy,
{
...
impl<C, Q, R, A, B> FactUnder<C, Q, R> for (A, B)
where
    C: Copy,
    Q: Copy,
    A: FactUnder<C, Q, R>,
    B: FactUnder<C, Q, R>,
{
    fn check_under(self, ctx: C, binder: Q, db: &R) -> bool {
        self.0.check_under(ctx, binder, db) && self.1.check_under(ctx, binder, db)
    }
}
...
{
    fn check_under(self, ctx: C, binder: Q, db: &R) -> bool {
        self.is_none_or(|f| f.check_under(ctx, binder, db))
    }
}