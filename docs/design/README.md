...
| [`proposals/hol-inductive-types/`](./proposals/hol-inductive-types/)                                   | proposed | Standalone proposal for implementing recursive inductive types **inside HOL** on top of the current Pure/HOL split: classic typedef + definitions + derived theorem bundle in `covalence-hol`, with **polymorphic datatypes first** and **bytes as a later concrete codec layer**. |
