...
- 8 well-known HOL Light derived rules (SYM, DISCH, MP, GEN, SPEC,
  MK_COMB-as-cong, ABS-as-cong, ETA_AX) are also primitives —
  ergonomics + performance, soundness justified by the standard
  HOL Light derivation in each docstring.
- A single non-computational axiom: **Peano induction on `nat`**.
...
Everything else — `pred`, `natRec`, addition, multiplication, division,
integer induction, `¬p ⇔ (p ⟹ F)`, conjunction introduction — is
derivable from `nat_induction` + the rule set + `define`. Until those
derivations land downstream (slated for the WASM-based proof rewrite),
consumers postulate them via `Thm::assume(body)`. Each such
postulate carries its body as a self-hyp so the audit chain is
...
├── hol.rs             — HOL-connective constructors (hol_eq/hol_imp/hol_forall/…)
│                        and the nat_induction axiom term
...
### 5.7 The single kernel axiom

```rust
Thm::nat_induction() -> Thm
    // ⊢ ∀P:nat→bool. P 0 ∧ (∀n:nat. P n ⟹ P (succ n)) ⟹ ∀n:nat. P n
    // Sound because the kernel commits to Type::nat() denoting the
    // standard naturals generated by 0 and succ.
```

**That is the entire non-computational axiom surface.** Every other
arithmetic/logic fact is derivable from this + the rule set + define.
...
These commitments are what makes `nat_induction` sound, what makes
`reduce_prim`'s `T = F → F` sound, etc.
