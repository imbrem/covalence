...
- **Abstract S-expr API — slice 1 landed, slices 2–5 open**
  (design + migration plan: `notes/vibes/lisp/abstract-sexpr-api.md`; landed:
  `covalence_sexp::abstract_sexpr` + `src/carrier.rs` + the
  `semantics.rs`/`relation.rs` helper delegation + `tests/carrier.rs`). Open:
  - **Phase-3 integration** (files owned by the concurrent book agent this
    round): route `session.rs` rendering through `CarvedCarrier` and delete
    the `Session.render_sem` borrowed-helpers hack; route `acl2.rs`'s
    `datum`/`render_data`/`as_acons`/`as_aint_arg`/`as_asym_arg` through
    `Acl2Carrier` (its `quote` already mirrors `datum` exactly, pinned by
    `tests/carrier.rs`).
  - **Slices 2–4** (repl-core): `Driven` + blanket `ReductionStrategy` impl
    (retiring `hol.rs::SymbolicStrategy` + starting the `Lisp`-trait
    deprecation); the `Composer` split + `RelationalSemantics`;
    `CertifiedEval`/`OrElse`/`Transport`. The §3.1 metacircular two-dialect
    demonstration needs these.
  - **Slice 5** is the existing `carved → sexpr` rename entry below.
