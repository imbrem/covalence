...
#[cfg(not(target_family = "wasm"))]
fn cmd_decide(args: DecideArgs) -> eyre::Result<()> {
    use covalence_kernel::Decision;
    ...
    for path in &args.files {
        ...
        let wasm = if is_wat {
            ...
            covalence_wasm::validate_wat(text)
        ...
        let decision = backend.decide(&hash).map_err(|e| eyre::eyre!("{e}"))?;
        let decide_elapsed = t_decide.elapsed();

        match decision {
            Decision::True => println!("{hash} true"),
            Decision::False => println!("{hash} false"),
            Decision::Unknown => {} // silent for unknown
        }
