...
- **[`src/spectec`](./src/spectec/SKELETONS.md)** — high-level SpecTec-fragment API (`Fragment` trait; `RelationEnv` peer of `GrammarEnv`; basic WASM semantics derived): only `Rel`+`Gram` kinds, no `Typ`/`Dec`, coverage = engine coverage.
