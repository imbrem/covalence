...
> [`metatheory.md`](../logics/metatheory.md) §7 (handler dispatch). The
> signature/theory/model architecture and the consumability matrix are
> sharpened in [`theories-models-and-logics.md`](../logics/theories-models-and-logics.md);
...
*itself* reified in HOL ([`metatheory.md`](../logics/metatheory.md) §1) — **models recurse
...
- A **first-order** theory quantifies only over individuals (its sorts), with
  ...
  *maximum reach* (this is the formal backbone of the "weakest sufficient
  logic = broadest applicability" idea, [`metatheory.md`](../logics/metatheory.md) §2).
...
[`metatheory.md`](../logics/metatheory.md) §7.2 / [`frontend.md`](./frontend.md) §4: a
...
`covalence-pure` literal-as-lifted-observation mechanism, `pure-design.md §3`,
...
we actually run ([`metatheory.md`](../logics/metatheory.md) §1). It is the model where the
...
- It is where `covalence-pure`'s `IsThm(theHOL)`
  ([`kernel-design.md`](../kernel/kernel-design.md) §11.2) is reified — the HOL observer
  lifted into a model.
- It is the base case for **HOL-to-X transport**: `HOL(A) ⟹ ZFC(A)`,
  `PA(A) ⟹ HOL(A)` ([`metatheory.md`](../logics/metatheory.md) §3) all speak about
...
mechanism *is* `covalence-pure` ([`kernel-design.md`](../kernel/kernel-design.md) §11):
...
2. **It is the `covalence-pure` lift, so it is trust-honest.** A lifter is a
   trusted observer ([`observers.md`](../observers/observers.md) §7): efficient (the kernel's
   built-in `Nat`/`Int`/`Blob` literals are the fast representation) but, under
   paranoid mode ([`kernel-design.md`](../kernel/kernel-design.md) §11.5), demotable to a
