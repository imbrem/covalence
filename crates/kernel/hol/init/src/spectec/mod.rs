...
/// engine's [`Premise`](crate::metalogic::binary::Premise): `Side` for an
/// antecedent proved outside the derivability predicate (a grammar terminal's
/// `Matches` leaf, a relation rule's side condition), `Derivation` for a
/// recursive sub-derivation.
pub enum FragPremise {
    /// A sub-derivation `⊢ <predicate> <j>` (a recursive premise).
    Derivation(Thm),
    /// A side theorem discharged directly (grammar terminal `Matches` leaf, or
    /// a relation side-condition antecedent — e.g. proved by computation via
    /// [`side_cond`]).
    Side(Thm),
}
