...
pub fn derive_under(env: &Acl2Env, hyps: &[Term], steps: &[Step], goal: &Term) -> Result<Thm> {
    ...
    while !open.is_empty() {
        lines = discharge_last(env, &open, lines)?;
        open.pop();
    }
...
mod tests {
    ...
    use crate::init::acl2::derivable::{
        derive_ind, finite_sigma, s6_env, subst_ground, transport_equal_open,
    };
    ...
    /// The S6+APP session (the gate env), shared across tests — its
    /// 50-clause soundness (incl. the IND discharge) is proved once.
    fn s6_app_session() -> &'static Acl2Session {
    ...
    // ------------------------------------------------------------------
    // The app-assoc object proofs (design §11) + THE S6 GATE (§12)
    // ------------------------------------------------------------------

    /// A step-list builder tracking line formulas (so the trans/symm
    /// axiom instances can be computed as it goes).
    struct B<'e> {
        env: &'e crate::init::acl2::derivable::Acl2Env,
        st: Vec<Step>,
        phis: Vec<Term>,
    }

    impl<'e> B<'e> {
        fn new(env: &'e crate::init::acl2::derivable::Acl2Env) -> Self {
            B {
                env,
                st: vec![],
                phis: vec![],
        ...
        fn trans_u(&mut self, jab: usize, jbc: usize) -> usize {
        ...
        fn symm_u(&mut self, jab: usize) -> usize {
            let (a, b) = equal_parts(self.env, &self.phis[jab]).unwrap();
    ...
    /// The shared app-assoc terms over the gate env.
    struct AppAssoc {
        phi: Term,   // (EQUAL (APP (APP X Y) Z) (APP X (APP Y Z)))
        g: Term,     // (EQUAL (CONSP X) 'NIL)
        c: Term,     // (CONSP X)
        ihcar: Term, // φ[X ↦ (CAR X)]
        ihcdr: Term, // φ[X ↦ (CDR X)]
    }

    fn app_assoc_terms(env: &crate::init::acl2::derivable::Acl2Env) -> AppAssoc {
        let tm = &*env.tm;
        let x = tm.sym(b"X").unwrap();
        let y = tm.sym(b"Y").unwrap();
    ...
    fn app_unfold_at(b: &mut B, l_hyp: usize, w: &Term, base: bool) -> usize {
        let env = b.env;
        let tm = &*env.tm;
        let x = tm.sym(b"X").unwrap();
        let consp_x = tm.app(b"CONSP", &[x.clone()]).unwrap();
    ...
    fn app_cons_lemma(
        env: &crate::init::acl2::derivable::Acl2Env,
        a: &Term,
        d: &Term,
        z: &Term,
    ) -> Fact {
    ...
    fn app_assoc_base(env: &crate::init::acl2::derivable::Acl2Env, t: &AppAssoc) -> Thm {
        let tm = &*env.tm;
        let y = tm.sym(b"Y").unwrap();
        let z = tm.sym(b"Z").unwrap();
    ...
    /// the tree-induction clause).
    fn app_assoc_step(env: &crate::init::acl2::derivable::Acl2Env, t: &AppAssoc) -> Thm {
        let tm = &*env.tm;
        let x = tm.sym(b"X").unwrap();
        let y = tm.sym(b"Y").unwrap();
        let z = tm.sym(b"Z").unwrap();
        ...
        let hyps = [t.c.clone(), t.ihcar.clone(), t.ihcdr.clone()];
        let mut b = B::new(env);
        let l_c = b.hyp(0, &t.c);
...
fn discharge_last(env: &Acl2Env, open: &[Term], lines: Vec<Line>) -> Result<Vec<Line>> {
    let tm = &*env.tm;
    ...
    for line in lines {
        ...
        let ni = match line.j {
            ...
            LineJ::Hyp(i) => {
                ...
                let li = out.len() - 1;
                let k = axiom_inst(env, "prop-k", &[(b"X", open[i].clone()), (b"Y", h.clone())])?;
                out.push(Line {
                    phi: k.phi,
                    j: LineJ::Fact(k.thm),
                });
            ...
            LineJ::Fact(t) => {
                ...
                let lf = out.len() - 1;
                let k = axiom_inst(env, "prop-k", &[(b"X", line.phi), (b"Y", h.clone())])?;
                out.push(Line {
                    phi: k.phi,
                    j: LineJ::Fact(k.thm),
                });
            ...
            LineJ::Mp(j0, k0) => {
                ...
                let s = axiom_inst(
                    env,
                    "prop-s",
                    &[(b"X", h.clone()), (b"Y", p.clone()), (b"Z", q.clone())],
                )?;
                out.push(Line {
                    phi: s.phi,
                    j: LineJ::Fact(s.thm),
