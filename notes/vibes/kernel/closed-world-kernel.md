...
The *trust surface* is a **closed, enumerable set of rules** rather than "trust
each context's crate".
...
values; the **sort of an expression is an associated type** (each expression type
has exactly one interpretation — you can't read it two ways). There is **no `Sort`
trait**: any Rust type is a sort the moment it is wrapped in `Val`.
...
**The equality calculus is universal and lives in the framework TCB (ungated);
only *computation* is gated by `admits` and appears in the manifest.**
