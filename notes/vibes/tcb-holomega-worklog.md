...
## 2026-07-12 — Haskell web demo connected to the kernel, end-to-end (main ae874845)

Zero-TCB (web-kernel is not a TCB crate; core/eval manifests byte-identical). The
`covalence-web-kernel` wasm already links the full kernel, so this was mostly wiring +
enabling features:
- `haskell_to_sexpr` (untyped SExpr interchange text), `haskell_to_hol_term` (carved
  `sexpr` kernel `Term` via the `hol` backend), `haskell_to_typed_hol` (annotated
  def/expr → well-typed HOL `Term` + its type via `hol-typed`/`covalence-hol-api`
  `NativeHol`), and **`check_haskell_proofs`** (dialect theorem module + separate S-expr
  proof file → per-theorem Proved/Axiom/Hole/Mismatch via the proof-format linker,
  replayed through the kernel). All `#[wasm_bindgen]`, JSON `{ok,…}` boundary; 13 native
  tests (incl. the nat_theorems proved/hole + wrong-proof→mismatch).
- **Web**: `/haskell` is LIVE (edit Haskell → SExpr IR + carved kernel Term + typed HOL
  Term panes). NEW **`/proofs`** page: theorem-module + proof-file editors → live
  colour-coded proof-check report (seeded with nat_theorems); nav link added.
- Cleanup: web-kernel JSON boundary factored into shared `json_*` helpers; module doc =
  surface index; per-fn docs tightened (typed vs untyped). SKELETONS updated.
- Verified: 13 web-kernel native tests; `bun run build:web-kernel` (wasm32 + hol-typed)
  green; `bun run build:web` green with all four `haskell_*` fns in the regenerated glue
  + built bundle; clippy/fmt clean; manifests byte-identical. Deferral: the proof-check
  driver types statement vars as `nat` (demo convention) — general multi-sort driver open.
  The gitignored wasm glue is regenerated by the build (not committed), per repo convention.
