(component
    ...
    (core module $m
        ...
        (func $start
            ...
            (local $sig i32)
            (local $principal i32)
            ;; Generate signer, get two different blobs
            (local.set $signer (call $generate_signer))
            (local.set $blob_a (call $get_blob_a))
            (local.set $blob_b (call $get_blob_b))
            ;; Sign blob_a
            (local.set $sig (call $signer_sign (local.get $signer) (local.get $blob_a)))
            ;; Get principal
            (local.set $principal (call $signer_principal (local.get $signer)))
            ;; Verify against blob_b — should be false (0)
            (if (i32.eqz (call $sig_verify (local.get $sig) (local.get $principal) (local.get $blob_b)))
                (then (call $attest))
            )
            ;; Cleanup
            ...
            (call $blob_drop (local.get $blob_a))
            (call $blob_drop (local.get $blob_b))
        )
        (start $start)
    )
