...
- **covalence-core** — **the TCB together with `covalence-pure(-trusted)`** (safe Rust; `core::Thm` is a newtype over `pure::Thm` — see `docs/deps/tcb.json`). HOL-Light-style kernel, locally-nameless `Term`/`Type`. Logical primitives are only `=` (`TermKind::Eq`) and `ε` (`TermKind::Select`); connectives are defined constants in `defs/logic.rs`. HOL Light's 10 primitives + fast derived constructors with `Soundness:` docstrings; `define` + `new_type_definition`; `unfold_term_spec` + the per-family computation certificate rules (`thm/certs.rs` over `covalence-pure-eval`; `reduce_prim`/`builtins.rs` deleted in purge S8 — the untrusted driver is `covalence-hol-eval::reduce`); `spec_abs`/`spec_rep`. Four non-computational primitive rules: `nat_induct`, `false_elim`, `unit_eq`, `lem`. (The observer rules `obs_eq`/`obs_true`/`obs_imp` were deleted in the toHOL purge; `Obs` leaves remain only as `new_type_definition` freshness tokens.) `defs/` catalogue. **Read `notes/vibes/kernel-design.md` before touching.**
...
- **covalence-init** — the semi-trusted API over `covalence-core`: the `init/` theory catalogue, `proofs/` tactics, the `script/` `.cov` layer + `project`, `metalogic`/`peano` (Metamath bridge), `models`, `grammar/{regex,spectec}` grammars, the algebra theories, `hash`/`sexp`. Depends on `covalence-hol` and re-exports its surface (so `covalence_init::{HolLightCtx,traits,types,Term,…}` resolve).
