...
- **covalence-wasm** — wraps `wat`, `wasmparser`, `wasmprinter`, `wasm-encoder`, optionally `wasmtime` (`runtime` feature; re-exported via `engine::wasmtime`). `ModuleBuilder`, `Val`/`ValType`.
...
- **covalence-inductive** (`crates/lang/inductive`) — the logic-agnostic **inductive-types API**: plain-data `InductiveSpec<X>` (simple = single/non-indexed/strictly-positive/first-order), two-tier `Logic`/`LogicOps` abstraction, `InductiveTheory`/`InductiveFacts` bundle (membership-relativized contract, iteration recursor, rule-form schematic theorems, honest `BackendCaps`), `InductiveBackend` seam, generic `conformance` suite. Deps: `smol_str`+`thiserror` only. HOL backends live in `covalence-init::init::inductive::{api,church,engine,carved}`: impredicative Church (`church`) = **universal, no `cdr`** (any spec incl. the `sexpr` flagship, catamorphic recursor, `rec_injective=false`); `carved` = **exact, full-Lisp** (the `atom|snil|scons` shape via `new_type_definition`, paramorphic recursor + `car`/`cdr`/rec-injectivity); engine adapter (`engine`) = exact `nat`; a set.mm backend slots in later. The **Lisp theory** (`init::lisp`, `Lisp`) is the first carved consumer — `car`/`cdr`/`cons`/`consp`/`atom?`/`len`/`append` + `append_assoc`/`len_append` by structural induction (ACL2 groundwork). Design + universal-vs-exact split + `proof/acl2` frontend map: `notes/vibes/inductive-api-design.md` (§4, §10).
...
- **covalence-core** — **the pure-HOL tier of the TCB, together with `covalence-pure(-trusted)`** (safe Rust; `core::Thm<L: HolTier = CoreLang>` is a newtype over `pure::Thm` — see `docs/deps/tcb.json`; `Thm<CoreLang>` carries NO computation axioms). HOL-Light-style kernel, locally-nameless `Term`/`Type`. Logical primitives are only `=` (`TermKind::Eq`) and `ε` (`TermKind::Select`); connectives — including `T`/`F` themselves since EG3b (`tru ≡ (λp.p)=(λp.p)`, `fal ≡ ∀p:bool.p`) — are defined constants in `defs/logic.rs`. HOL Light's equality-core primitives + a few fast derived constructors with `Soundness:` docstrings (25 rules, golden `docs/deps/core-manifest.txt` — the connective/quantifier rules, `lem` (L2) and `false_elim` (EG3b) left the kernel: they are zero-TCB, tier-generic derivations in `covalence-hol-eval::derived::DerivedRules`, same signatures, working at `Thm<CoreLang>` and `EvalThm` alike; the transitional `Bool` literals `⌜T⌝`/`⌜F⌝` bridge to the defined `T`/`F` via derived eval-tier equations in `covalence-hol-eval::boolean`); `define` + `new_type_definition` (returns the bijection as ONE unsplit conjunction — split via eval's `TypeDefExt`); `unfold_term_spec`; `spec_abs`/`spec_rep`. Two non-computational primitive rules: `nat_induct` (sequent form), `unit_eq`. `src/defs/` is only the **D3 residue** (literal-leaf type handles + connective builders — dies with the literal leaves; see its `SKELETONS.md`): the term-op catalogue and the cert rules moved to `covalence-hol-eval` (stage E2). **Read `notes/vibes/kernel-design.md` before touching.**
- **covalence-hol-eval** (`crates/kernel/hol/eval`) — **the eval tier**: owns `CoreEval` (a `Language` extending `CoreLang`) + `EvalThm = Thm<CoreEval>`; hosts the per-family computation certificate rules + toHOL rules (`rules.rs`, 17 rules incl. `FloatCert`, golden `docs/deps/eval-manifest.txt`) NEXT TO their `defs/` dispatch tables and the term-op catalogue (the auditability goal — `defs/floats.rs` = the two-layer float architecture: F2b bit-level declarations + the F2c typed `f64.*` let-style wrappers). Trust is per-rule via `admits`; the drivers (`reduce`/`prove_true`/`delta`, the `lit.rs` facade, the `typed_float.rs` composite that reduces typed float ops with NO new rule) stay untrusted. `derived.rs` (stage L2) = the zero-TCB connective/quantifier rules (`DerivedRules` — drop-in signatures for the deleted kernel rules incl. `lem` via a cached ε-schema; `TypeDefExt` splits the typedef bijection); eval-tier only because `⊢ T` needs the cert path. GOTCHA: `reduce`'s normal forms are load-bearing for hand-assembled `trans` chains in covalence-init — never widen its catalogue over existing symbols (new opt-in passes instead; see `init/ball.rs`'s `ball_eval` + `ext::fire_all`). `tests/pure_hol_units.rs` = the definition-vs-native differential per cert family (dev-dep on covalence-init). `core -> hol-eval` and normal `hol-eval -> init` are BANNED dep-graph edges.
...
- **covalence-kernel** (`crates/kernel/core`) — the "OS-kernel" layer over the logical TCB: sits between `covalence-init` and `covalence-shell`, wiring the HOL proof world to durable infrastructure. Re-exports `covalence_init`; `backend` (`SyncBackend`/`AsyncBackend`/`KernelError`), `store` (a content-addressed `Kernel` over the blob store), `service` (`KernelService`), and a skeleton `facts` module (the observer layer lands here). See `crates/kernel/core/src/lib.rs`.
- **covalence-hol-api** (`crates/kernel/hol/api`) — the TRAIT-backed consumer surface (non-TCB, zero delta): `Hol` (value-typed HOL Light), `Nat`, `Int` (the abstracted integer API — builders + destructors + ordered-ring lemma accessors), `HolOmega`; `NativeHol` is the only impl naming the concrete backend. Point: swap the backend / integer representation by writing one impl. Consumed by lang/lisp, lang/haskell, kernel/web, and (planned) the SMT replay.
- **covalence-kernel-sat** (`crates/kernel/sat`) — generic SAT-proof replay (UNTRUSTED, zero TCB): the `ClauseBackend` resolution seam + `replay_lrat`, turning a DRAT/LRAT refutation (from lib/sat) into backend theorems. The propositional substrate for the SMT replay.
- **covalence-kernel-smt** (`crates/kernel/smt`) — generic SMT-proof replay (UNTRUSTED, zero TCB): the pure Farkas certificate checker (`rational`/`lincomb`/`farkas` — Alethe `la_generic` linear algebra, backend-independent) + `RulePolicy` (admit only a subset of Alethe). Written over `covalence-hol-api::Int`, so it retargets HOL / alt-int-reps / internal-PA / SOA. Turns a parsed Alethe proof (lib/smt) into `⊢ ⊥`. See `notes/vibes/logics/smt-import-architecture.md`.

## Proof-format frontends

- **covalence-alethe** — Alethe (SMT) proof checking → HOL (`AletheBridge` + `HolAletheBridge` + `ingest` + `la.rs` Farkas + `(#by (smt))` tactic). The working HOL bridge; its generic, backend-parameterised successor is `covalence-kernel-smt` (migration pending).
- **covalence-egglog** — egglog integration (currently disabled behind an `external-egglog` feature — released egglog still lacks the `proof` module; see the crate's `Cargo.toml`).
