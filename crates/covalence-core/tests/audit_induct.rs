...
//! (`Thm::nat_induct`, `Thm::false_elim`, `Thm::unit_eq`), the observer
//! rules (`Thm::obs_eq`, `Thm::obs_true`, `Thm::obs_imp`), and the
...
//! - `obs_*`: unconditionally sound under the parametric ε-model
//!   regardless of the policy return; the kernel still enforces the
//!   shape/type/downcast preconditions, which we test directly.
...
// ============================================================================
// Observer rules: obs_eq / obs_true / obs_imp
// ============================================================================
//
// These rules are unconditionally sound under the parametric ε-model
// regardless of the policy return value (see ObsEq/ObsTrue/ObsImp
// docstrings). The kernel still enforces shape/type/downcast
// preconditions, which we exercise here with a tiny test observer.

/// A trivial observer whose policy methods return a fixed answer.
#[derive(Debug)]
struct TestObs {
    answer: bool,
}

impl ObsEq for TestObs {
    fn obs_eq(
        &self,
        _other: &Self,
        _my: &[Term],
        _other_args: &[Term],
        _hint: Option<&dyn Hint>,
    ) -> bool {
        self.answer
    }
}

impl ObsTrue for TestObs {
    fn obs_true(&self, _args: &[Term], _hint: Option<&dyn Hint>) -> bool {
        self.answer
    }
}

impl ObsImp for TestObs {
    fn obs_imp(&self, _args: &[Term], _hyps: &[Term], _hint: Option<&dyn Hint>) -> bool {
        self.answer
    }
}

/// A *different* observer Rust type, to test downcast rejection.
#[derive(Debug)]
struct OtherObs;
...
impl ObsTrue for OtherObs {
    fn obs_true(&self, _args: &[Term], _hint: Option<&dyn Hint>) -> bool {
        true
    }
}
impl ObsImp for OtherObs {
    fn obs_imp(&self, _args: &[Term], _hyps: &[Term], _hint: Option<&dyn Hint>) -> bool {
        true
    }
}
...
#[test]
fn obs_eq_happy_path_when_policy_accepts() {
    // Two bool-typed observation leaves of the same Rust type.
    let e1 = Term::obs(TestObs { answer: true }, Type::bool());
    let e2 = Term::obs(TestObs { answer: true }, Type::bool());
...
#[test]
fn obs_eq_rejected_when_policy_declines() {
    let e1 = Term::obs(TestObs { answer: false }, Type::bool());
    let e2 = Term::obs(TestObs { answer: false }, Type::bool());
    assert!(Thm::obs_eq::<TestObs>(e1, e2, None).is_err());
}

#[test]
fn obs_eq_rejects_type_mismatch_between_sides() {
    let e1 = Term::obs(TestObs { answer: true }, Type::bool());
    let e2 = Term::obs(TestObs { answer: true }, Type::nat());
    assert!(
        Thm::obs_eq::<TestObs>(e1, e2, None).is_err(),
        "different Core types must be rejected before policy"
    );
}

#[test]
fn obs_eq_rejects_wrong_observer_type() {
    // The requested type O = OtherObs does not match the leaves' TestObs.
    let e1 = Term::obs(TestObs { answer: true }, Type::bool());
    let e2 = Term::obs(TestObs { answer: true }, Type::bool());
    assert!(Thm::obs_eq::<OtherObs>(e1, e2, None).is_err());
}

#[test]
fn obs_eq_rejects_non_obs_head() {
    // A plain (non-obs) term has no obs head.
    let plain = Term::bool_lit(true);
    let e2 = Term::obs(TestObs { answer: true }, Type::bool());
    assert!(Thm::obs_eq::<TestObs>(plain, e2, None).is_err());
}
...
#[test]
fn obs_true_happy_path() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    let thm = Thm::obs_true::<TestObs>(e.clone(), None).unwrap();
    assert_eq!(thm.concl(), &e);
}

#[test]
fn obs_true_rejected_when_policy_declines() {
    let e = Term::obs(TestObs { answer: false }, Type::bool());
    assert!(Thm::obs_true::<TestObs>(e, None).is_err());
}

#[test]
fn obs_true_rejects_non_bool_result() {
    // obs result type is nat, not prop/bool.
    let e = Term::obs(TestObs { answer: true }, Type::nat());
    assert!(Thm::obs_true::<TestObs>(e, None).is_err());
}

#[test]
fn obs_true_rejects_wrong_observer_type() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    assert!(Thm::obs_true::<OtherObs>(e, None).is_err());
}

#[test]
fn obs_true_rejects_non_obs_head() {
    assert!(Thm::obs_true::<TestObs>(Term::bool_lit(true), None).is_err());
}

// ---- obs_imp ----

#[test]
fn obs_imp_happy_path_builds_implication_chain() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    let h0 = Term::free("h0", Type::bool());
    let h1 = Term::free("h1", Type::bool());
    let thm = Thm::obs_imp::<TestObs>(e.clone(), vec![h0.clone(), h1.clone()], None).unwrap();
    // Expect ⊢ h0 ⟹ h1 ⟹ e (right-associative).
    let expected = hol_imp(h0, hol_imp(h1, e));
    assert_eq!(thm.concl(), &expected);
}

#[test]
fn obs_imp_no_hyps_is_just_the_obs() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    let thm = Thm::obs_imp::<TestObs>(e.clone(), vec![], None).unwrap();
    assert_eq!(thm.concl(), &e, "empty hyp chain reduces to the consequent");
}

#[test]
fn obs_imp_rejected_when_policy_declines() {
    let e = Term::obs(TestObs { answer: false }, Type::bool());
    assert!(Thm::obs_imp::<TestObs>(e, vec![], None).is_err());
}

#[test]
fn obs_imp_rejects_non_bool_hyp() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    let bad_hyp = zero(); // nat, not bool
    assert!(Thm::obs_imp::<TestObs>(e, vec![bad_hyp], None).is_err());
}

#[test]
fn obs_imp_rejects_non_bool_consequent() {
    let e = Term::obs(TestObs { answer: true }, Type::nat());
    assert!(Thm::obs_imp::<TestObs>(e, vec![], None).is_err());
}

#[test]
fn obs_imp_rejects_wrong_observer_type() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    assert!(Thm::obs_imp::<OtherObs>(e, vec![], None).is_err());
}

// ---- obs leaf marks the theorem as non-trivial ----

#[test]
fn obs_true_theorem_has_obs_leaf() {
    let e = Term::obs(TestObs { answer: true }, Type::bool());
    let thm = Thm::obs_true::<TestObs>(e, None).unwrap();
    assert!(
        !thm.has_no_obs(),
        "an obs-derived theorem must NOT be flagged oracle-free"
    );
    assert!(thm.all_obs_match::<TestObs>(), "all obs leaves are TestObs");
}

/// Sanity that the unused `Arc<dyn Hint>` import path compiles: a hint
/// can be threaded through and is ignored by the policy.
#[test]
fn obs_eq_accepts_hint() {
    let e1 = Term::obs(TestObs { answer: true }, Type::bool());
    let e2 = Term::obs(TestObs { answer: true }, Type::bool());
    let hint: Arc<dyn Hint> = Arc::new(42u64);
    assert!(Thm::obs_eq::<TestObs>(e1, e2, Some(hint)).is_ok());
}
