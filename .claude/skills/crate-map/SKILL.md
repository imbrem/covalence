...
- **covalence-k** (`crates/lang/k`) — the K-framework frontend. Untrusted, kernel-free, `#![forbid(unsafe_code)]`. **Two independent frontends:** (1) **KORE** (`ast`/`parse`/`sexpr`/`fragment`) — consumes the elaborated matching-logic IR kompile emits as `definition.kore` (as covalence-spectec consumes elaborated SpecTec AST): parser (mangled identifiers, multiary `\and`/`\or`, sortless `\mu`/`\nu`), canonical S-expr IR, and the fragment classifier (rewrite/function/subsort/constructor/… ladder, `RewriteRule` extraction). (2) **`.k`-source grammar** (`kdef`) — reads a K definition's `syntax` (BNF) declarations directly, skipping `rule`/`configuration`/… sentences: a **winnow**-based parser (`kdef::parse`, via `covalence_parse::winnow`) for modules/imports/productions/`|`/`>` priority/`[attrs]`/`List{}`, a canonical S-expr IR (`kdef::sexpr`), and a lowering to `covalence_grammar::Cfg` (`kdef::cfg`) that feeds the kernel CFG stratum. Imported builtin sorts (`Id`/`Int`/`Bool` from DOMAINS-SYNTAX) resolve via the swappable `cfg::SortResolver` trait (`NoDomains` = structural, `KDomains` = K builtin token regexes). Parses the real LAMBDA/IMP tutorial grammars (vendored BSD-3 in `examples/k-tutorial/`). Deps: covalence-sexp, covalence-grammar, covalence-parse, smol_str, thiserror. The F0 KORE→`Derivable_KStep` lowering lives in `covalence-init::k`; design `notes/design/k-frontend.md`.
