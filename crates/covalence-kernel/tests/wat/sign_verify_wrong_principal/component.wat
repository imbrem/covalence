(component
    ...
    (core module $m
        ...
        (func $start
            ...
            ;; Sign with A
            (local.set $sig (call $signer_sign (local.get $signer_a) (local.get $blob)))
            ;; Get B's principal
            (local.set $principal_b (call $signer_principal (local.get $signer_b)))
            ;; Verify with B's principal — should be false (0)
            (if (i32.eqz (call $sig_verify (local.get $sig) (local.get $principal_b) (local.get $blob)))
                (then (call $attest))
            )
            ;; Cleanup
            (call $signer_drop (local.get $signer_a))
            (call $signer_drop (local.get $signer_b))
            (call $principal_drop (local.get $principal_b))
