...
- `SliceEnv` (`wasm::lower::slice`, total-load Wave F) is the third `Fragment`
  impl: the same ergonomics over a **spec slice** — a premise-closed subset of
  the total combined clause list — with `transport` lifting slice theorems
  into the full set (`spectec-total-load.md` § *Spec slices + transport*).
  Also re-exported from `covalence-hol-api`.
