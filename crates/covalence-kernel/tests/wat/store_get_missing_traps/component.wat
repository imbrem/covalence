;; Store get missing traps: get root, call get("missing") without setting. Trap before attest.
(component
    ...
    (core module $m
        ...
        ;; Data: key = "missing" at offset 0
        (data (i32.const 0) "missing")

        (func $start
            (local $handle i32)
            (local.set $handle (call $root))
            ;; get("missing") → will trap (key not found)
            (call $store_get (local.get $handle) (i32.const 0) (i32.const 7) (i32.const 64))
