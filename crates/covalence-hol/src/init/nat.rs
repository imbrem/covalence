...
pub fn natrec_env() -> crate::script::Env {
    ...
    // the Euclidean division algorithm — proved in Rust (strong induction +
    // floor-witness transfer) and handed to `nat.cov` as givens. `div.spec`
    // is the two-clause selector body transferred to `nat.div`; `mod.def` its
    // `let`-unfolding; `div.mod` / `mod.lt` the algorithm proper.
    e.define_lemma("div.spec", div_spec());
    e.define_lemma("mod.def", mod_def());
    e.define_lemma("div.mod", div_mod());
    e.define_lemma("mod.lt", mod_lt());
    e
}

/// The **full basic natural-numbers toolkit** as one importable environment:
/// the proved `nat.cov` theorems MERGED with the `natrec` givens (the
/// recursion theorem `rec.holds`, the `+`/`*`/`-`/`pred`/`pow`/`shl`/`shr`
/// recursion equations, the `≤`/`<` selector clauses, and the Euclidean
/// division algorithm `div.spec`/`mod.def`/`div.mod`/`mod.lt`) plus the nat
/// operator constants (`nat.add`, `nat.mul`, …, `nat.div`, `nat.mod`, `succ`).
///
/// This is what `(#import nat)` resolves to (see [`crate::init::library_env`]):
/// a single `(#open nat)` then exposes `natrec`, every operator, and every
/// proved fact — no separate `core`/`natrec` imports needed for the basics.
pub fn nat_toolkit_env() -> crate::script::Env {
    use crate::script::ConstDef;
    let mut e = cov::env();
    // The recursion theorem + raw recursion equations + order clauses + the
    // division givens (so downstream proofs can recurse on `natRec` directly).
    e.merge(&natrec_env());
    // The nat operator constants (otherwise only reachable through `core`).
    let mut op = |names: &[&str], t: Term| {
        for n in names {
            e.define_const((*n).to_string(), ConstDef::Op(t.clone()));
        }
    };
...
crate::cov_theory! {
    ...
    pub mod cov from "nat.cov" {
        ...
        "mod.unfold"   => pub fn mod_unfold_cov;
        "div_mod"      => pub fn div_mod_cov;
        "mod_lt"       => pub fn mod_lt_cov;
    }
}
...
// ============================================================================
// Strong / course-of-values induction — derived from `Thm::nat_induct`.
//
// `(∀n. (∀k. k < n ⟹ P k) ⟹ P n) ⟹ ∀n. P n`. The prerequisite for the
// Euclidean division algorithm (`div` is `⌊n/m⌋`, defined by recursion on
// `n` with a *smaller-than-`n`* recursive call `(n - m)/m`).
//
// Proof: let `Acc n ≜ ∀k. k < n ⟹ P k`. We show `∀n. Acc n` by ordinary
// induction (`Acc 0` is vacuous — nothing is `< 0`; `Acc n ⟹ Acc (S n)`
// because `k < S n` means `k < n` (use `Acc n`) or `k = n` (use the step at
// `n`, fed `Acc n`)). Then `P n` for every `n` is `step n (Acc n)`.
// ============================================================================

/// `p k`, the motive `p : nat → bool` applied to `k` (NOT β-reduced — the
/// applied form `Thm::nat_induct` / the `induct` helper consume).
fn p_at(p: &Term, k: Term) -> Term {
    Term::app(p.clone(), k)
}

/// `∀k. k < n ⟹ p k` — the strong-induction accumulator at `n`.
fn acc_at(p: &Term, n: &Term) -> Result<Term> {
    lt_t(var("k"), n.clone())
        .imp(p_at(p, var("k")))?
        .forall("k", nat())
}

/// **Strong (course-of-values) induction.** From
/// `step : ⊢ ∀n. (∀k. k < n ⟹ p k) ⟹ p n` conclude `⊢ ∀n. p n`, where
/// `p : nat → bool` is the motive (the `p n` are in *applied* form). Genuine
/// — built from `Thm::nat_induct` plus the `<`/`≤` order theory, no
/// hypotheses beyond `step`'s.
pub fn strong_induct(p: &Term, step: Thm) -> Result<Thm> {
    // motive for the ordinary induction: Acc ≜ λn. ∀k. k < n ⟹ p k.
    let acc_motive = Term::abs(nat(), subst::close(&acc_at(p, &var("n"))?, "n"));

    // base: Acc 0 = ∀k. k < 0 ⟹ p k — vacuous (nothing is < 0).
    let base = {
        let k = var("k");
        let k_lt_0 = lt_t(k.clone(), zero());
        not_lt_zero()
            .all_elim(k.clone())?
            .not_elim(Thm::assume(k_lt_0.clone())?)? // {k<0} ⊢ F
            .false_elim(p_at(p, k.clone()))? // {k<0} ⊢ p k
            .imp_intro(&k_lt_0)? // ⊢ k < 0 ⟹ p k
            .all_intro("k", nat())?
    };

    // step: Acc n ⟹ Acc (S n).
    let n = var("n");
    let ihc = acc_at(p, &n)?; // ∀k. k < n ⟹ p k
    let inner = {
        let k = var("k");
        let acc_n = Thm::assume(ihc.clone())?;
        // From Acc n derive p n via the strong-induction step at n.
        let p_n = step.clone().all_elim(n.clone())?.imp_elim(acc_n.clone())?; // {Acc n} ⊢ p n
        // Goal: ∀k. k < S n ⟹ p k. Fix k, assume k < S n.
        let k_lt_sn = lt_t(k.clone(), succ(n.clone()));
        // k < S n  ⟺  k ≤ n  ⟺  k < n ∨ k = n.
        let k_le_n = lt_iff_succ_le()
            .all_elim(k.clone())?
            .all_elim(succ(n.clone()))? // (k < S n) = (S k ≤ S n)
            .trans(le_succ_succ().all_elim(k.clone())?.all_elim(n.clone())?)? // = (k ≤ n)
            .eq_mp(Thm::assume(k_lt_sn.clone())?)?; // {k<Sn} ⊢ k ≤ n
        let disj = le_iff_lt_or_eq()
            .all_elim(k.clone())?
            .all_elim(n.clone())?
            .eq_mp(k_le_n)?; // {k<Sn} ⊢ (k < n) ∨ (k = n)
        // left k<n: use Acc n at k.
        let on_lt = acc_n
            .all_elim(k.clone())?
            .imp_elim(Thm::assume(lt_t(k.clone(), n.clone()))?)? // {Acc n, k<n} ⊢ p k
            .imp_intro(&lt_t(k.clone(), n.clone()))?;
        // right k=n: rewrite p n along k = n.
        let keqn = k.clone().equals(n.clone())?;
        let on_eq = Thm::refl(p.clone())?
            .cong_app(Thm::assume(keqn.clone())?.sym()?)? // (p n) = (p k)  [from n = k]
            .eq_mp(p_n.clone())? // {Acc n, k=n} ⊢ p k
            .imp_intro(&keqn)?;
        let p_k = disj.or_elim(on_lt, on_eq)?; // {Acc n, k<Sn} ⊢ p k
        p_k.imp_intro(&k_lt_sn)?
            .all_intro("k", nat())? // {Acc n} ⊢ ∀k. k < S n ⟹ p k
            .imp_intro(&ihc)? // ⊢ Acc n ⟹ Acc (S n)
    };
    let all_acc = induct(&acc_motive, base, inner)?; // ⊢ ∀n. Acc n

    // ⊢ ∀n. p n: for each n, step n (Acc n).
    let n2 = var("n");
    let acc_n2 = beta_reduce(all_acc.all_elim(n2.clone())?)?; // ⊢ ∀k. k < n ⟹ p k
    step.all_elim(n2.clone())?
        .imp_elim(acc_n2)? // ⊢ p n
        .all_intro("n", nat())
}
...
// needs a *witness* floor function. We build it by **Hilbert choice over the
// existence theorem** ([`div_exists`], proved by [`strong_induct`] on `n`):
// `w ≜ λn m. ε q. dpred n m q`, then transfer the predicate to `nat.div`
// itself with `spec_ax` (`transfer_div`). From the transferred bounds the
// division algorithm [`div_mod`] / [`mod_lt`] and the recurrences follow.
...
// ---- the division algorithm: existence, witness transfer, div_mod / mod_lt ----

/// `S q · m` — the upper-bound multiplier in the division predicate.
fn succ_mul(q: Term, m: Term) -> Term {
    mul(succ(q), m)
}

/// `(q·m ≤ n) ∧ (n < S q · m)` — the Euclidean bounds at quotient `q`.
fn div_bounds_body(q: &Term, n: &Term, m: &Term) -> Result<Term> {
    le_t(mul(q.clone(), m.clone()), n.clone()).and(lt_t(n.clone(), succ_mul(q.clone(), m.clone())))
}

/// `(m = 0 ⟹ q = 0) ∧ (¬(m = 0) ⟹ bounds)` — the choice predicate body at
/// a concrete quotient `q` (this is exactly the `nat.div` predicate's body,
/// instantiated at `d n m := q`).
fn dpred_body(q: &Term, n: &Term, m: &Term) -> Result<Term> {
    let m0 = m.clone().equals(zero())?;
    let case0 = m0.clone().imp(q.clone().equals(zero())?)?;
    let casep = m0.not()?.imp(div_bounds_body(q, n, m)?)?;
    case0.and(casep)
}

/// `dpred n m ≜ λq. dpred_body q n m` — the per-`(n,m)` choice predicate, as
/// a `nat → bool` λ ready for `ε` / `select_ax`.
fn dpred(n: &Term, m: &Term) -> Result<Term> {
    Ok(Term::abs(nat(), subst::close(&dpred_body(&var("q"), n, m)?, "q")))
}

cached_thm! {
    /// `⊢ ∀n m. (0 < m) ⟹ (m ≤ n) ⟹ (n - m < n)` — subtracting a positive
    /// `m ≤ n` strictly decreases. (`m ≤ n` gives `m + (n-m) = n`; `0 < m`
    /// gives `1 ≤ m`, so `S(n-m) = 1+(n-m) ≤ m+(n-m) = n`, i.e. `n-m < n`.)
    pub fn sub_lt() -> Result<Thm> {
        let (n, m) = (var("n"), var("m"));
        let h_pos = lt_t(zero(), m.clone());
        let h_le = le_t(m.clone(), n.clone());
        let d = sub(n.clone(), m.clone()); // n - m
        // m + (n-m) = n.
        let m_d_eq_n = le_add_sub()
            .all_elim(m.clone())?
            .all_elim(n.clone())?
            .imp_elim(Thm::assume(h_le.clone())?)?; // {m≤n} ⊢ m + (n-m) = n
        // 1 ≤ m  (from 0 < m).
        let one_le_m = lt_iff_succ_le()
            .all_elim(zero())?
            .all_elim(m.clone())?
            .eq_mp(Thm::assume(h_pos.clone())?)?; // {0<m} ⊢ S 0 ≤ m
        // S0 + (n-m) ≤ m + (n-m)  by right add-monotonicity of ≤.
        let mono = le_add_cancel_r()
            .all_elim(succ(zero()))?
            .all_elim(m.clone())?
            .all_elim(d.clone())?
            .sym()?
            .eq_mp(one_le_m)?; // {0<m} ⊢ S0 + (n-m) ≤ m + (n-m)
        // S0 + (n-m) = S(n-m).
        let s0d = add_step()
            .all_elim(zero())?
            .all_elim(d.clone())? // S0 + (n-m) = S(0 + (n-m))
            .trans(add_base().all_elim(d.clone())?.cong_arg(nat_succ())?)?; // = S(n-m)
        // S(n-m) ≤ m+(n-m) = n, i.e. n - m < n.
        let s_d_le_n = mono.rewrite(&s0d)?.rewrite(&m_d_eq_n)?; // {0<m, m≤n} ⊢ S(n-m) ≤ n
        lt_iff_succ_le()
            .all_elim(d.clone())?
            .all_elim(n.clone())?
            .sym()?
            .eq_mp(s_d_le_n)? // n - m < n
            .imp_intro(&h_le)?
            .imp_intro(&h_pos)?
            .all_intro("m", nat())?
            .all_intro("n", nat())
...
cached_thm! {
    /// `⊢ ∀n m. ∃q. dpred n m q` — **existence of the Euclidean quotient**.
    /// For `m = 0` the witness is `q = 0`; for `m ≠ 0` it is `⌊n/m⌋`, built
    /// by strong induction on `n`: if `n < m` then `q = 0`, else `q = S q'`
    /// for the quotient `q'` of `n - m` (which is `< n` by [`sub_lt`], so the
    /// strong IH applies).
    pub fn div_exists() -> Result<Thm> {
        let m = var("m");
        // motive P n ≜ ∃q. dpred n m q   (m free during the strong induction).
        let body_at = |n: &Term| -> Result<Term> {
            dpred_body(&var("q"), n, &m)?.exists("q", nat())
        };
        let p = Term::abs(nat(), subst::close(&body_at(&var("n"))?, "n"));

        // strong step: ∀n. (∀k. k < n ⟹ P k) ⟹ P n.
        let n = var("n");
        let acc = acc_at(&p, &n)?;
        let ih = Thm::assume(acc.clone())?; // ∀k. k < n ⟹ P k
        // Case on m = 0 ∨ ¬(m=0).
        let m0 = m.clone().equals(zero())?;
        // -- m = 0: witness q = 0. dpred_body 0 n 0.
        let on_m0 = {
            let assume_m0 = Thm::assume(m0.clone())?;
            // case0: (m=0 ⟹ 0=0) holds (refl), discharged under m=0.
            let case0 = Thm::refl(zero())?.imp_intro(&m0)?;
            // casep: ¬(m=0) ⟹ bounds — antecedent contradicts assume_m0.
            let not_m0 = m0.clone().not()?;
            let casep = Thm::assume(not_m0.clone())?
                .not_elim(assume_m0.clone())?
                .false_elim(div_bounds_body(&zero(), &n, &m)?)?
                .imp_intro(&not_m0)?;
            let wbody = case0.and_intro(casep)?; // ⊢ dpred_body 0 n m   (q := 0)
            // ∃q. dpred n m q, witness 0.
            let pred = dpred(&n, &m)?;
            let beta = Thm::beta_conv(Term::app(pred.clone(), zero()))?; // (dpred)0 = dpred_body 0
            let wapp = beta.sym()?.eq_mp(wbody)?; // ⊢ (dpred n m) 0
            exists_intro(pred, zero(), wapp)?.imp_intro(&m0)?
        };
        // -- ¬(m=0): split n < m vs m ≤ n via le_total / trichotomy.
        let not_m0 = m0.clone().not()?;
        let on_not_m0 = {
            let assume_not_m0 = Thm::assume(not_m0.clone())?;
            // 0 < m  from ¬(m=0): m = 0 ∨ 0 < m  (le_iff_lt_or_eq on 0 ≤ m).
            let pos_m = {
                // (0 ≤ m) = (0 < m) ∨ (0 = m).
                let decomp = le_iff_lt_or_eq()
                    .all_elim(zero())?
                    .all_elim(m.clone())?
                    .eq_mp(le_zero().all_elim(m.clone())?)?; // ⊢ (0 < m) ∨ (0 = m)
                let on_lt = Thm::assume(lt_t(zero(), m.clone()))?
                    .imp_intro(&lt_t(zero(), m.clone()))?;
                let on_eq = {
                    let eq0m = zero().equals(m.clone())?;
                    let m_eq_0 = Thm::assume(eq0m.clone())?.sym()?; // m = 0
                    assume_not_m0
                        .clone()
                        .not_elim(m_eq_0)? // F  (¬(m=0) applied)
                        .false_elim(lt_t(zero(), m.clone()))?
                        .imp_intro(&eq0m)?
                };
                decomp.or_elim(on_lt, on_eq)? // {¬(m=0)} ⊢ 0 < m
            };
            // n < m ∨ m ≤ n  via le_total (m ≤ n ∨ n ≤ m) + decomposition,
            // but simpler: use trichotomy on n,m to get (n<m) ∨ (m≤n).
            let n_lt_m = lt_t(n.clone(), m.clone());
            let m_le_n = le_t(m.clone(), n.clone());
            // (n ≤ m) ∨ (m ≤ n) from le_total; we actually want (n<m)∨(m≤n).
            // Use: le_total m n gives (m≤n) ∨ (n≤m); on n≤m, decompose to n<m ∨ n=m;
            // on n=m, m≤n by le_refl-transport. Build (n<m) ∨ (m≤n).
            let case_split = {
                let tot = le_total().all_elim(m.clone())?.all_elim(n.clone())?; // (m≤n)∨(n≤m)
                let on_mle = Thm::assume(m_le_n.clone())?
                    .or_intro_r(n_lt_m.clone())? // (n<m)∨(m≤n)
                    .imp_intro(&m_le_n)?;
                let on_nle = {
                    let n_le_m = le_t(n.clone(), m.clone());
                    let dec = le_iff_lt_or_eq()
                        .all_elim(n.clone())?
                        .all_elim(m.clone())?
                        .eq_mp(Thm::assume(n_le_m.clone())?)?; // (n<m)∨(n=m)
                    let l = Thm::assume(n_lt_m.clone())?
                        .or_intro_r(m_le_n.clone())?
                        .imp_intro(&n_lt_m)?;
                    let r = {
                        let neqm = n.clone().equals(m.clone())?;
                        // n = m ⟹ m ≤ n: rewrite the 2nd arg of `m ≤ m` (le_refl)
                        // from m to n using m = n.
                        Thm::refl(Term::app(nat_le(), m.clone()))? // ⊢ nat.le m = nat.le m
                            .cong_app(Thm::assume(neqm.clone())?.sym()?)? // (m≤m) = (m≤n)
                            .eq_mp(le_refl().all_elim(m.clone())?)? // ⊢ m ≤ n
                            .or_intro_r(n_lt_m.clone())?
                            .imp_intro(&neqm)?
                    };
                    dec.or_elim(l, r)?.imp_intro(&n_le_m)?
                };
                tot.or_elim(on_mle, on_nle)? // {¬(m=0)} ⊢ (n<m) ∨ (m≤n)
            };
            // on n < m: witness q = 0.  0·m = 0 ≤ n,  n < m = S0·m.
            let on_nltm = {
                // S 0 · m = m.
                let s0m_eq_m = mul_step()
                    .all_elim(zero())?
                    .all_elim(m.clone())? // S0·m = m + 0·m
                    .trans(
                        mul_base()
                            .all_elim(m.clone())?
                            .cong_arg(Term::app(nat_add(), m.clone()))?, // = m + 0
                    )?
                    .trans(add_zero().all_elim(m.clone())?)?; // = m
                // bounds at q=0:  0·m ≤ n  ∧  n < S0·m.
                let lower = Thm::refl(nat_le())?
                    .cong_app(mul_base().all_elim(m.clone())?)? // (0·m ≤ n) = (0 ≤ n)
                    .cong_fn(n.clone())?
                    .sym()?
                    .eq_mp(le_zero().all_elim(n.clone())?)?; // ⊢ 0·m ≤ n
                let upper = Thm::refl(nat_lt())?
                    .cong_fn(n.clone())?
                    .cong_app(s0m_eq_m.sym()?)? // (n < m) = (n < S0·m)
                    .eq_mp(Thm::assume(n_lt_m.clone())?)?; // {n<m} ⊢ n < S0·m
                let bounds = lower.and_intro(upper)?; // {n<m} ⊢ bounds 0
                let case0 = assume_not_m0
                    .clone()
                    .not_elim(Thm::assume(m0.clone())?)?
                    .false_elim(zero().equals(zero())?)?
                    .imp_intro(&m0)?;
                let casep = bounds.imp_intro(&not_m0)?;
                let wbody = case0.and_intro(casep)?; // dpred_body 0 n m
                let pred = dpred(&n, &m)?;
                let beta = Thm::beta_conv(Term::app(pred.clone(), zero()))?;
                let wapp = beta.sym()?.eq_mp(wbody)?;
                exists_intro(pred, zero(), wapp)?.imp_intro(&n_lt_m)?
            };
            // on m ≤ n: recurse on n - m.
            let on_mlen = {
                let d = sub(n.clone(), m.clone()); // n - m
                // n - m < n  (sub_lt with 0<m, m≤n).
                let d_lt_n = sub_lt()
                    .all_elim(n.clone())?
                    .all_elim(m.clone())?
                    .imp_elim(pos_m.clone())?
                    .imp_elim(Thm::assume(m_le_n.clone())?)?; // {…} ⊢ n-m < n
                // P (n - m) from the strong IH.
                let p_d = beta_reduce(ih.clone().all_elim(d.clone())?.imp_elim(d_lt_n)?)?; // ∃q. dpred (n-m) m q
                // Eliminate the ∃: given q' with dpred (n-m) m q', build dpred n m (S q').
                let qp = var("q'");
                let inner_pred_app = Term::app(dpred(&d, &m)?, qp.clone()); // (dpred (n-m) m) q'
                // step for exists_elim: ∀q'. (dpred(n-m) m) q' ⟹ ∃q. dpred n m q.
                let step = {
                    let body_q = beta_reduce(Thm::assume(inner_pred_app.clone())?)?; // dpred_body q' (n-m) m
                    // ¬(m=0) holds (assume_not_m0), so project the bounds for q'.
                    let bounds_qp = body_q
                        .and_elim_r()?
                        .imp_elim(assume_not_m0.clone())?; // q'·m ≤ (n-m) ∧ (n-m) < S q'·m
                    let lo = bounds_qp.clone().and_elim_l()?; // q'·m ≤ n-m
                    let hi = bounds_qp.and_elim_r()?; // (n-m) < S q'·m
                    // Target quotient S q'.  S q' · m = m + q'·m.
                    let sqm_eq = mul_step().all_elim(qp.clone())?.all_elim(m.clone())?; // S q'·m = m + q'·m
                    // lower:  S q'·m ≤ n.  m + q'·m ≤ m + (n-m) = n.
                    let m_d_eq_n = le_add_sub()
                        .all_elim(m.clone())?
                        .all_elim(n.clone())?
                        .imp_elim(Thm::assume(m_le_n.clone())?)?; // m + (n-m) = n
                    let lower = {
                        // q'·m ≤ n-m  ⟹  m + q'·m ≤ m + (n-m).
                        let mono = le_add_cancel_r()
                            .all_elim(mul(qp.clone(), m.clone()))?
                            .all_elim(d.clone())?
                            .all_elim(m.clone())?
                            .sym()?
                            .eq_mp(lo.clone())?; // q'·m + m ≤ (n-m) + m
                        // rewrite to m + q'·m ≤ n  via comm + m_d_eq_n.
                        mono
                            .rewrite(&add_comm().all_elim(mul(qp.clone(), m.clone()))?.all_elim(m.clone())?)? // m + q'·m ≤ (n-m)+m
                            .rewrite(&add_comm().all_elim(d.clone())?.all_elim(m.clone())?)? // m+q'·m ≤ m+(n-m)
                            .rewrite(&m_d_eq_n)? // m + q'·m ≤ n
                            .rewrite(&sqm_eq.clone().sym()?)? // S q'·m ≤ n
                    };
                    // upper:  n < S(S q')·m.  n = m + (n-m) < m + S q'·m = S(S q')·m.
                    let upper = {
                        // (n-m) < S q'·m  ⟹  m + (n-m) < m + S q'·m.
                        let mono = lt_add_mono_r()
                            .all_elim(d.clone())?
                            .all_elim(succ_mul(qp.clone(), m.clone()))?
                            .all_elim(m.clone())?
                            .sym()?
                            .eq_mp(hi.clone())?; // (n-m)+m < (S q'·m)+m
                        // rewrite to n < m + S q'·m.
                        let lhs_to_n = mono
                            .rewrite(&add_comm().all_elim(d.clone())?.all_elim(m.clone())?)? // m+(n-m) < (Sq'·m)+m
                            .rewrite(&m_d_eq_n)?; // n < (S q'·m)+m
                        // (S q'·m)+m = m + S q'·m = S(S q')·m.
                        let rhs_eq = add_comm()
                            .all_elim(succ_mul(qp.clone(), m.clone()))?
                            .all_elim(m.clone())? // (Sq'·m)+m = m + S q'·m
                            .trans(mul_step().all_elim(succ(qp.clone()))?.all_elim(m.clone())?.sym()?)?; // = S(Sq')·m
                        lhs_to_n.rewrite(&rhs_eq)? // n < S(S q')·m
                    };
                    let bounds = lower.and_intro(upper)?; // bounds (S q') at n,m
                    let case0 = assume_not_m0
                        .clone()
                        .not_elim(Thm::assume(m0.clone())?)?
                        .false_elim(succ(qp.clone()).equals(zero())?)?
                        .imp_intro(&m0)?;
                    let casep = bounds.imp_intro(&not_m0)?;
                    let wbody = case0.and_intro(casep)?; // dpred_body (S q') n m
                    let pred = dpred(&n, &m)?;
                    let beta = Thm::beta_conv(Term::app(pred.clone(), succ(qp.clone())))?;
                    let wapp = beta.sym()?.eq_mp(wbody)?; // (dpred n m)(S q')
                    exists_intro(pred, succ(qp.clone()), wapp)?
                        .imp_intro(&inner_pred_app)?
                        .all_intro("q'", nat())? // ∀q'. (dpred(n-m) m) q' ⟹ ∃q. dpred n m q
                };
                exists_elim(p_d, body_at(&n)?, step)?.imp_intro(&m_le_n)?
            };
            case_split.or_elim(on_nltm, on_mlen)?.imp_intro(&not_m0)?
        };
        // Combine the two cases with excluded middle on (m = 0).
        let p_n = Thm::lem(m0.clone())?.or_elim(on_m0, on_not_m0)?; // {acc} ⊢ P n (β-expanded below)
        let p_n_app = beta_expand(&p, n.clone(), p_n)?; // {acc} ⊢ p n
        let step = p_n_app.imp_intro(&acc)?.all_intro("n", nat())?;
        strong_induct(&p, step)?.all_intro("m", nat())
    }
}

/// `⊢ pred (ε pred)` from `⊢ ∃x. pred x` (`pred : nat → bool`). The Hilbert
/// choice principle: an inhabited predicate is satisfied at its ε-choice. Each
/// `exists_elim` branch applies `Thm::select_ax(pred, q)` at the bound witness.
fn select_from_exists(pred: &Term, ex: Thm) -> Result<Thm> {
    let q = var("q");
    let eps = Term::app(Term::select_op(nat()), pred.clone()); // ε pred
    let p_eps = Term::app(pred.clone(), eps); // pred (ε pred)
    // step: ∀q. pred q ⟹ pred (ε pred).
    let step = Thm::select_ax(pred.clone(), q.clone())?.all_intro("q", nat())?;
    exists_elim(ex, p_eps, step)
}

/// `λn m. ε q. dpred n m q` — the floor witness extracted from [`div_exists`]
/// by Hilbert choice.
fn div_witness() -> Result<Term> {
    let n = var("n");
    let m = var("m");
    let eps = Term::app(Term::select_op(nat()), dpred(&n, &m)?); // ε (dpred n m)
    let lam_m = Term::abs(nat(), subst::close(&eps, "m"));
    Ok(Term::abs(nat(), subst::close(&lam_m, "n")))
}

cached_thm! {
    /// `⊢ ∀n m. (m = 0 ⟹ nat.div n m = 0) ∧
    ///          (¬(m = 0) ⟹ (nat.div n m · m ≤ n ∧ n < S(nat.div n m) · m))`
    /// — the Euclidean **division bounds**, transferred to `nat.div` itself
    /// from the floor witness ([`div_witness`]) via `spec_ax`. Genuine and
    /// hypothesis-free: [`div_exists`] inhabits the choice predicate at every
    /// `(n, m)`, `select_ax` pins `ε` to a satisfying quotient, and the
    /// def-style `spec_ax(nat.div, w)` transfers the body to `nat.div`.
    pub fn div_spec() -> Result<Thm> {
        let n = var("n");
        let m = var("m");
        let w = div_witness()?;
        // `dpred n m (w n m)` for the witness — from select_ax + div_exists.
        // w n m = ε (dpred n m), so select_ax gives (∃ ⟹) dpred n m (ε …).
        let pred = dpred(&n, &m)?;
        // (∃q. dpred n m q) ⟹ dpred n m (ε (dpred n m)).
        // `div_exists` is `∀m. ∀n. (λn. ∃q. dpred_body q n m) n` — eliminate
        // `m` then `n`, then β-reduce the single `(λn. …) n` redex.
        let ex_nm = beta_reduce(
            div_exists().all_elim(m.clone())?.all_elim(n.clone())?,
        )?; // ∃q. dpred n m q
        // select form: from ∃, get pred (ε pred).
        let sel = select_from_exists(&pred, ex_nm)?; // ⊢ dpred n m (ε (dpred n m))
        // (w n m) = ε (dpred n m), definitionally: w n m β-reduces to ε(dpred n m).
        let wnm = Term::app(Term::app(w.clone(), n.clone()), m.clone());
        let wnm_eq = beta_nf_concl(Thm::refl(wnm.clone())?)?; // ⊢ w n m = ε(dpred n m)  (β-nf both sides equal)
        // sel is `(dpred n m)(ε…)` = `dpred_body (ε…) n m` after β. Fold ε… back
        // to `w n m` and β-expand to the predicate-application form spec_ax wants.
        let sel_body = beta_reduce(sel)?; // dpred_body (ε(dpred n m)) n m
        // rewrite ε(dpred n m) ↦ w n m throughout the body.
        let sel_at_w = sel_body.rewrite(&wnm_eq.clone().sym()?)?; // dpred_body (w n m) n m
        let body_w = sel_at_w.all_intro("m", nat())?.all_intro("n", nat())?; // ∀n m. dpred_body (w n m) n m

        // Now transfer to nat.div via spec_ax. `body_w` is `BODY[w]` where BODY
        // is exactly nat.div's selector predicate; `transfer_selector`-style.
        transfer_selector_div(w, body_w)
    }
}

/// Transfer a proof of `BODY[w]` (the `nat.div` selector body at witness `w`)
/// to `BODY[nat.div]` via `spec_ax(nat.div, w)`. `body_w` must already be the
/// fully-quantified `∀n m. dpred_body (w n m) n m`.
fn transfer_selector_div(w: Term, body_w: Thm) -> Result<Thm> {
    let imp = Thm::spec_ax(nat_div(), w)?; // ⊢ p w ⟹ p nat.div
    let pw = imp
        .concl()
        .as_app()
        .and_then(|(f, _)| f.as_app())
        .map(|(_, ante)| ante.clone())
        .ok_or_else(|| Error::ConnectiveRule("transfer_div: bad spec_ax shape".into()))?;
    // `p w` β-reduces to `∀n m. dpred_body (w n m) n m` — bridge body_w to it.
    let pw_proof = Thm::beta_conv(pw)?.sym()?.eq_mp(body_w)?; // ⊢ p w
    let p_div = imp.imp_elim(pw_proof)?; // ⊢ p nat.div
    Thm::beta_conv(p_div.concl().clone())?.eq_mp(p_div) // ⊢ BODY[nat.div]
}

/// `nat.div n m · m ≤ n` — for any `m` (when `m = 0`, `nat.div n m = 0` so the
/// product is `0 ≤ n`; when `m ≠ 0`, the lower bound of [`div_spec`]).
fn div_mul_le_at(n: &Term, m: &Term) -> Result<Thm> {
    let spec = div_spec().all_elim(n.clone())?.all_elim(m.clone())?;
    let m0 = m.clone().equals(zero())?;
    let dnm = nat_div_t(n.clone(), m.clone());
    // Case m = 0.
    let on_m0 = {
        let div0 = spec.clone().and_elim_l()?.imp_elim(Thm::assume(m0.clone())?)?; // {m=0} ⊢ nat.div n m = 0
        // nat.div n m · m = 0 · m  (rw div) ... but simpler: rewrite via div0 and m=0.
        // (nat.div n m · m ≤ n) target: rewrite nat.div n m ↦ 0 and m ↦ 0.
        // 0 · 0 = 0 ≤ n.
        let prod_eq = Thm::refl(nat_mul())?
            .cong_app(div0.clone())? // mul (nat.div n m) = mul 0
            .cong_app(Thm::assume(m0.clone())?)?; // (nat.div n m · m) = (0 · 0)
        let z = prod_eq
            .trans(mul_base().all_elim(zero())?)?; // nat.div n m · m = 0
        Thm::refl(nat_le())?
            .cong_app(z.clone())? // (nat.div n m · m ≤ n) = (0 ≤ n)
            .cong_fn(n.clone())?
            .sym()?
            .eq_mp(le_zero().all_elim(n.clone())?)? // {m=0} ⊢ nat.div n m · m ≤ n
            .imp_intro(&m0)?
    };
    // Case m ≠ 0.
    let not_m0 = m0.clone().not()?;
    let on_not_m0 = spec
        .and_elim_r()?
        .imp_elim(Thm::assume(not_m0.clone())?)?
        .and_elim_l()? // {¬(m=0)} ⊢ nat.div n m · m ≤ n
        .imp_intro(&not_m0)?;
    let _ = dnm;
    Thm::lem(m0)?.or_elim(on_m0, on_not_m0)
}

cached_thm! {
    /// `⊢ ∀n m. n = (n / m) · m + (n mod m)` — **the division algorithm**.
    /// Holds *unconditionally* (`n / 0 = 0`, `n mod 0 = n`, so the RHS is
    /// `0 + n = n`): `n mod m = n − (n/m)·m` ([`mod_def`]) and
    /// `(n/m)·m ≤ n` ([`div_mul_le_at`]), so [`le_add_sub`] collapses
    /// `(n/m)·m + (n − (n/m)·m)` to `n`.
    pub fn div_mod() -> Result<Thm> {
        let n = var("n");
        let m = var("m");
        let q_m = mul(nat_div_t(n.clone(), m.clone()), m.clone()); // (n/m)·m
        // (n/m)·m + (n mod m) = (n/m)·m + (n − (n/m)·m)   [mod_def]
        let md = mod_def().all_elim(n.clone())?.all_elim(m.clone())?; // n mod m = n − (n/m)·m
        let step1 = md.cong_arg(Term::app(nat_add(), q_m.clone()))?; // (n/m)·m + (n mod m) = (n/m)·m + (n−(n/m)·m)
        // = n   [le_add_sub with (n/m)·m ≤ n]
        let le = div_mul_le_at(&n, &m)?; // (n/m)·m ≤ n
        let step2 = le_add_sub()
            .all_elim(q_m.clone())?
            .all_elim(n.clone())?
            .imp_elim(le)?; // (n/m)·m + (n−(n/m)·m) = n
        step1.trans(step2)?
            .sym()? // n = (n/m)·m + (n mod m)
            .all_intro("m", nat())?
            .all_intro("n", nat())
...
cached_thm! {
    /// `⊢ ∀n m. ¬(m = 0) ⟹ n mod m < m` — the **remainder bound**. From the
    /// upper bound `n < S(n/m)·m = (n/m)·m + m` ([`div_spec`]) and
    /// `(n/m)·m + (n mod m) = n` ([`div_mod`]): `(n/m)·m + (n mod m) <
    /// (n/m)·m + m`, so `n mod m < m` by left add-cancellation.
    pub fn mod_lt() -> Result<Thm> {
        let n = var("n");
        let m = var("m");
        let not_m0 = m.clone().equals(zero())?.not()?;
        let q = nat_div_t(n.clone(), m.clone());
        let qm = mul(q.clone(), m.clone()); // (n/m)·m
        // upper bound: n < S(n/m)·m.
        let upper = div_spec()
            .all_elim(n.clone())?
            .all_elim(m.clone())?
            .and_elim_r()?
            .imp_elim(Thm::assume(not_m0.clone())?)?
            .and_elim_r()?; // {¬(m=0)} ⊢ n < S(n/m)·m
        // S(n/m)·m = m + (n/m)·m   [mul_step].
        let sqm_eq = mul_step().all_elim(q.clone())?.all_elim(m.clone())?; // S q·m = m + q·m
        // n < m + (n/m)·m.
        let n_lt = upper.rewrite(&sqm_eq)?; // n < m + (n/m)·m
        // n = (n/m)·m + (n mod m)   [div_mod].
        let dm = div_mod().all_elim(n.clone())?.all_elim(m.clone())?; // n = (n/m)·m + (n mod m)
        // (n/m)·m + (n mod m) < m + (n/m)·m = (n/m)·m + m.
        let lt1 = n_lt
            .rewrite(&dm)? // (n/m)·m + (n mod m) < m + (n/m)·m
            .rewrite(&add_comm().all_elim(m.clone())?.all_elim(qm.clone())?)?; // < (n/m)·m + m
        // left-cancel (n/m)·m:  (q + x < q + y) = (x < y) via comm + lt_add_mono_r.
        let nmod = nat_mod_t(n.clone(), m.clone());
        let cancelled = lt1
            .rewrite(&add_comm().all_elim(qm.clone())?.all_elim(nmod.clone())?)? // (n mod m)+ (n/m)·m < (n/m)·m + m
            .rewrite(&add_comm().all_elim(qm.clone())?.all_elim(m.clone())?)?; // (n mod m)+q·m < m + q·m
        lt_add_mono_r()
            .all_elim(nmod.clone())?
            .all_elim(m.clone())?
            .all_elim(qm.clone())?
            .eq_mp(cancelled)? // n mod m < m
            .imp_intro(&not_m0)?
            .all_intro("m", nat())?
            .all_intro("n", nat())
    }
...
mod tests {
    ...
    #[test]
    fn division_algorithm_is_genuine() {
        let (n, m) = (var("n"), var("m"));
        // div_mod: ⊢ ∀n m. n = (n/m)·m + (n mod m).
        let dm = div_mod()
            .all_elim(n.clone())
            .unwrap()
            .all_elim(m.clone())
            .unwrap();
        let q_m = mul(nat_div_t(n.clone(), m.clone()), m.clone());
        let want_dm = n
            .clone()
            .equals(add(q_m.clone(), nat_mod_t(n.clone(), m.clone())))
            .unwrap();
        assert_eq!(dm.concl(), &want_dm);
        // mod_lt: ⊢ ∀n m. ¬(m=0) ⟹ n mod m < m.
        let ml = mod_lt()
            .all_elim(n.clone())
            .unwrap()
            .all_elim(m.clone())
        ...
        let want_ml = m
            ...
            .not()
            .unwrap()
            .imp(lt_t(nat_mod_t(n.clone(), m.clone()), m.clone()))
            .unwrap();
        assert_eq!(ml.concl(), &want_ml);
        // The whole chain — strong induction, existence, witness transfer — is
        // hypothesis-free.
        for t in [super::sub_lt(), super::div_exists(), super::div_spec(), div_mod(), mod_lt()] {
            assert!(t.hyps().is_empty(), "the division algorithm is genuine");
            assert!(t.concl().type_of().unwrap().is_bool());
        }
    }

    #[test]
    fn div_spec_bounds_transfer_to_nat_div() {
        // div_spec: the two clauses about `nat.div` itself.
        let (n, m) = (var("n"), var("m"));
        let spec = div_spec()
            .all_elim(n.clone())
            .unwrap()
            .all_elim(m.clone())
            .unwrap();
        // For m ≠ 0: (n/m)·m ≤ n  and  n < S(n/m)·m.
        let bounds = spec
            .and_elim_r()
            .unwrap()
            .imp_elim(Thm::assume(m.clone().equals(zero()).unwrap().not().unwrap()).unwrap())
            .unwrap();
        let dnm = nat_div_t(n.clone(), m.clone());
        assert_eq!(
            bounds.clone().and_elim_l().unwrap().concl(),
            &le_t(mul(dnm.clone(), m.clone()), n.clone())
        );
        assert_eq!(
            bounds.and_elim_r().unwrap().concl(),
            &lt_t(n.clone(), mul(succ(dnm), m.clone()))
        );
    }
    ...
    #[test]
    fn strong_induction_is_genuine() {
        // Use strong induction to reprove `∀n. 0 ≤ n` (the motive ignores the
        // strong IH — we only check the machinery runs and is hypothesis-free).
        let p = Term::abs(nat(), le_t(zero(), var("n"))); // λn. 0 ≤ n
        // step: ∀n. (∀k. k < n ⟹ 0 ≤ k) ⟹ 0 ≤ n — the consequent holds outright.
        let n = var("n");
        let acc = super::acc_at(&p, &n).unwrap();
        let step = beta_expand(&p, n.clone(), le_zero().all_elim(n.clone()).unwrap())
            .unwrap() // ⊢ p n   (i.e. 0 ≤ n, in applied form)
            .imp_intro(&acc)
            .unwrap()
            .all_intro("n", nat())
            .unwrap();
        let all_p = super::strong_induct(&p, step).unwrap();
        assert!(all_p.hyps().is_empty(), "strong induction is genuine");
        // ⊢ ∀n. p n; specialise + β-reduce to read `0 ≤ k`.
        let inst = beta_reduce(all_p.all_elim(var("k")).unwrap()).unwrap();
        assert_eq!(inst.concl(), &le_t(zero(), var("k")));
    }
...
mod cov_tests {
    ...
    fn nat_cov_loads_and_proves() {
        ...
        // Euclidean division / modulus ported to `nat.cov`.
        assert_eq!(cov::mod_unfold_cov().concl(), super::mod_def().concl());
        assert_eq!(cov::div_mod_cov().concl(), super::div_mod().concl());
        assert_eq!(cov::mod_lt_cov().concl(), super::mod_lt().concl());
    }
    ...
    #[test]
    fn nat_toolkit_gives_natrec_and_division_in_one_import() {
        // A downstream theory imports ONLY `nat` (the toolkit env) and `core`
        // (for the primitive tactics / `=` / literals), then uses `natrec`
        // (via the `rec.holds` recursion equations) AND the division algorithm
        // (`div.mod` / `mod.lt`) — all available from the single nat import.
        use crate::script::{Env, run};
        let resolver = |name: &str| match name {
            "core" => Some(Env::core()),
            "nat" => Some(super::nat_toolkit_env()),
            _ => None,
        };
        let theory = run(
            r#"
            ...
            (#import nat)
            (#open nat)
            ;; natRec is reachable: project its base equation out of `rec.holds`.
            (#thm natrec_base
              (#fix (z nat) (f (fun nat (fun nat nat))))
              (#concl (= (app (app (app (natrec-op nat) z) f) 0) z))
              (#proof (and-elim-l (all-elim f (all-elim z (rec.holds))))))
            ;; the division algorithm is reachable directly.
            (#thm div_algo
              (#fix (n nat) (m nat))
              (#concl (= n (nat.add (nat.mul (nat.div n m) m) (nat.mod n m))))
              (#proof (all-elim m (all-elim n (div.mod)))))
            ;; and the remainder bound.
            (#thm rem_bound
              (#fix (n nat) (m nat))
              (#concl (==> (not (= m 0)) (nat.lt (nat.mod n m) m)))
              (#proof (all-elim m (all-elim n (mod.lt)))))
            "#,
            resolver,
            |_| None,
        )
        .expect("the nat toolkit should expose natrec + division in one import");
        for t in &theory.thms {
            assert!(t.thm.hyps().is_empty(), "toolkit-derived facts are genuine");
        }
    }
}
