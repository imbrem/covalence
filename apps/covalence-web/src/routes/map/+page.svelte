<script lang="ts">
	...
	const allNodes = map.nodes as MapNode[];
	const allEdges = map.edges as MapEdge[];
	const byId = new Map(allNodes.map((node) => [node.id, node]));
	const tasks = allNodes.filter((node) => node.kind === 'task');
	const statuses = [...new Set(allNodes.map((node) => node.status).filter(Boolean))].sort() as string[];
	...
	let selectedId = $state<string | null>(tasks[0]?.id ?? null);
...
<main>
	<header>
		<div>
			<h1>Covalence map</h1>
			<p>
				Tasks, open work, notes, and code references from
				<code>docs/deps/covalence-map.json</code>. The snapshot is generated by
				<code>bun run deps</code> and embedded at build time.
			</p>
		</div>
		<div class="totals">
