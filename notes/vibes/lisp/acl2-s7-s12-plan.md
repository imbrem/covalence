...
specifies the entry points. **Status 2026-07-17: items 1–2 below are DONE**
(S5c + S5d landed — reports in `acl2-s5-design.md` §15.2/§15.3; the item-2
cross-check "app-assoc via IND-ORD with measure `(acl2-count x)`" is the
committed G4 gate `init/acl2/gate_s5d.rs`, whose obligation scripts are the
item-3 prototype). S7 proper starts at item 3.
