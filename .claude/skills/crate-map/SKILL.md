...
- **covalence-opentheory** — OpenTheory article/package verification. Generic stack machine `ArticleInterp<K: HolLightKernel>` (the trait is the backend-swap seam) + a native backend `NativeOt` (feature `native`) over `covalence-core` — zero TCB beyond core HOL (axioms are hypothesis-tracked `assume`s, never minted). Package/dep resolution + offline `FileResolver` / HTTP-cache `CachingUrlResolver` (feature `fetch`). Driven by `cov hol check <files.art>` / `cov hol pkg <packages>` and `bun run opentheory[:offline]` (download+cache+verify-all bench). The online resolver versions bare names via the repo's `?pkg=` page, so **the whole stdlib verifies**: `cov hol pkg base` = 1340 theorems (~46s cached). A reusable matching framework (`matching.rs`: `MatchLogic` × `MatchStrategy`) + native-override system (`NativeOverrides`/`OverrideMap`) discharge OpenTheory's axioms natively: `cov hol pkg --native-axioms base` proves infinity (over `nat`), extensionality, and choice in covalence's kernel → **1340 theorems relative to ZERO axioms** (`axioms.rs`). Design: `notes/vibes/logics/opentheory-import.md`; open work in the crate's `SKELETONS.md` (`.int` interpretations).
